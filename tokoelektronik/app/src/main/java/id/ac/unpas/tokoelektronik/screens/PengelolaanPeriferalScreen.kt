...
fun PengelolaanPeriferalScreen() {
    val _list = remember { MutableStateFlow(listOf<Periferal>()) }
    val list by remember { _list }.collectAsState()
    Column(modifier = Modifier.fillMaxWidth()) {
        FormPencatatanPeriferal { item ->
            ...
            items(items = list, itemContent = { item ->
                Row(modifier = Modifier
                    ...
                    Column(modifier = Modifier.weight(3f)) {
                        ...
                        Text(text = "${item.deskripsi} Kg", fontSize = 16.sp,
                            fontWeight = FontWeight.Bold)
                    }
                    Column(modifier = Modifier.weight(3f)) {
                        Text(text = "Berat", fontSize = 14.sp)
                        Text(text = "${item.jenis} Kg", fontSize = 16.sp,
                            fontWeight = FontWeight.Bold)
                    }
                }
